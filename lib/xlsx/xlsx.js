...
const {assertSafeMediaPath} = require('../utils/media-path-guard');
const {createDecompressionBudget, assertSafeZipEntry} = require('../utils/decompression-guard');
...
class XLSX {
  ...
  async load(data, options) {
    ...
    const decompressionBudget = createDecompressionBudget(options);
    for (const entry of Object.values(zip.files)) {
      ...
      if (!entry.dir) {
        ...
        assertSafeZipEntry(entry, entryName, decompressionBudget);
        let stream;
        if (
    ...
    await Promise.all(
      model.media.map(async medium => {
        if (medium.type === 'image') {
          ...
          if (medium.filename) {
            // [exceljs-hardened] CVE fix — arbitrary local file read.
            // Enforced again here (in addition to Workbook#addImage) so
            // any caller that builds `model.media` directly, bypassing
            // addImage(), still gets the same protection at the one
            // place that actually touches the filesystem.
            assertSafeMediaPath(medium.filename);
